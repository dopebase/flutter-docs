...
The first thing you need to do is install the camera plugin. Through the use of the camera plugin, we will be able to take photographs using the device’s built-in camera. Begin by adding a reliance on the camera in the our flutter project by running: 
