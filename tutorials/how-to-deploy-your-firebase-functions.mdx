...
3. From the root directory (`my-firebase-project`) run command: <code>firebase use</code> and check if the firebase project is selected to where you want to push this functions is selected. If not run command <code>firebase use --add</code> to select your firebase project.
...
4. Deploy the functions to your Firebase project:

```mdx
firebase deploy --only functions
```
5. By following these steps and ensuring you select the correct Firebase project, you'll have the Instaflutter-provided Firebase Functions up and running in your own Firebase environment, properly linked to your Flutter app and ready to support your app's backend needs.
